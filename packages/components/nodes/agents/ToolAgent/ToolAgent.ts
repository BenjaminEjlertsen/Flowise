  ...
  async run(
    ...
    if (shouldStreamResponse) {
      ...
      if (res.sourceDocuments) {
        if (sseStreamer) {
          sseStreamer.streamSourceDocumentsEvent(
            chatId,
            flatten(res.sourceDocuments),
          );
        }
      ...
      if (res.usedTools && res.usedTools.length) {
        ...
        for (const tool of res.usedTools) {
          const inputTool = inputTools.find(
            (inputTool: Tool) => inputTool.name === tool.tool,
          );
...
const prepareAgent = async (
  ...
  const sourceDocumentsContent = flowObj?.sourceDocuments
    ? flatten(flowObj.sourceDocuments)
  ...
  let promptVariables = {};
  const chatPromptTemplate = nodeData.inputs
    ?.chatPromptTemplate as ChatPromptTemplate;
  if (chatPromptTemplate && chatPromptTemplate.promptMessages.length) {
    const humanPrompt =
      chatPromptTemplate.promptMessages[
        chatPromptTemplate.promptMessages.length - 1
      ];
    const messages = [
  ...
  if (llmSupportsVision(model)) {
    ...
    const messageContent = await addImagesToMessages(
      nodeData,
      options,
      model.multiModalOption,
    );
    ...
    if (messageContent?.length) {
      ...
      let messagePlaceholder =
        prompt.promptMessages.pop() as MessagesPlaceholder;
      if (prompt.promptMessages.at(-1) instanceof HumanMessagePromptTemplate) {
        const lastMessage =
          prompt.promptMessages.pop() as HumanMessagePromptTemplate;
        const template = (lastMessage.prompt as PromptTemplate)
          .template as string;
  ...
  const runnableAgent = RunnableSequence.from([
    {
      ...
      [memoryKey]: async (_: { input: string; steps: ToolsAgentStep[] }) => {
        const messages = (await memory.getChatMessages(
          flowObj?.sessionId,
          true,
          prependMessages,
        )) as BaseMessage[];
